-- Yes probability history derived from real position volume (recorded on each trade).

create table public.market_yes_prices (
  id uuid primary key default gen_random_uuid(),
  market_id uuid not null references public.markets (id) on delete cascade,
  yes_probability integer not null check (
    yes_probability >= 0 and yes_probability <= 100
  ),
  recorded_at timestamptz not null default now()
);

create index market_yes_prices_market_id_recorded_at_idx
  on public.market_yes_prices (market_id, recorded_at asc);

-- Aggregate Yes share of total volume; 50 when no shares exist yet.
create or replace function public.market_yes_probability(p_market_id uuid)
returns integer
language sql
stable
security definer
...
as $$
  select coalesce(
    (
      select round(
        100.0 * sum(pos.yes_shares_cents)::numeric
        / nullif(sum(pos.yes_shares_cents + pos.no_shares_cents), 0)
      )::integer
...
-- One starting point per market so charts are never synthetic.
insert into public.market_yes_prices (market_id, yes_probability, recorded_at)
select m.id, 50, m.created_at
from public.markets m;

alter table public.market_yes_prices enable row level security;

create policy "Market yes prices are publicly readable"
  on public.market_yes_prices
  for select
  to anon, authenticated
  using (true);

grant select on public.market_yes_prices to anon, authenticated;
...
begin
  ...
  set balance_cents = prof.balance_cents - p_amount_cents
  where prof.id = v_user_id;
  ...
  insert into public.market_yes_prices (market_id, yes_probability)
  values (p_market_id, public.market_yes_probability(p_market_id));
