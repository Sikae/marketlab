-- Return market-wide Yes % after each trade so the UI can update outcomes immediately.

grant execute on function public.market_yes_probability(uuid) to anon, authenticated;

drop function if exists public.buy_market_shares(uuid, text, bigint);

create function public.buy_market_shares(
  p_market_id uuid,
  p_side text,
...
begin
  ...
  v_yes_probability := public.market_yes_probability(p_market_id);
